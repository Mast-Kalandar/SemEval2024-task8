...
class GRU_RoBERTA(nn.Module):
    def __init__(self,roberta_path = "roberta-base",lstm_hidden_size=256,lstm_layers=2,lstm_dropout=0.2 , num_classes=2):
        ...
        self.lstm = nn.GRU(self.roberta.config.hidden_size,lstm_hidden_size,lstm_layers,batch_first=True,dropout=lstm_dropout,bidirectional=True)
...
class LitModel(L.LightningModule):
    def __init__(self,train_loader , val_loader , model_path, lr=2e-5, hidden_size=256, lstm_layers=2, lstm_dropout=0.2, num_classes=2):
        ...
        self.model = GRU_RoBERTA(model_path, hidden_size, lstm_layers, lstm_dropout, num_classes)
...
# %%
wandb_logger = WandbLogger(project='roberta-gru', log_model=True , name="roberta-gru")
trainer = L.Trainer(max_epochs=5, devices="auto",val_check_interval=2000,default_root_dir="robert-gru",strategy='ddp_find_unused_parameters_true' , logger=wandb_logger)

# %%
model = LitModel( train_loader=train_loader , val_loader=test_loader , model_path="roberta-base" , lr=2e-5, hidden_size=256, lstm_layers=2, lstm_dropout=0.2, num_classes=2)

trainer.fit(model)
trainer.save_checkpoint("best_bert_freeze.ckpt")
# %%
